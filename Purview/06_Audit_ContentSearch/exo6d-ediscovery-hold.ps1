...
# ========================================================================================
# Concept : eDiscovery = processus légal de collecte et conservation de preuves numériques.
...
#                                  empêche toute suppression sur une mailbox SPÉCIFIQUE
#                                  pendant l'investigation.
#                                  Le hold prime sur toute politique de suppression automatique.
#
# ARCHITECTURE SESSION — point critique :
#   Les cmdlets eDiscovery (*-ComplianceCase, *-CaseHoldPolicy, *-CaseHoldRule)
#   ne sont chargées QUE si la session IPPSSession est ouverte avec -EnableSearchOnlySession.
#   Sans ce flag, les cmdlets sont introuvables ("not recognized as a name of a cmdlet")
#   même si le module est bien installé.
#   Toutes les opérations eDiscovery nécessitent ce flag — une seule session IPPS suffit.
#
...
#   2. Vérifie l'existence de la mailbox cible (via Connect-ExchangeOnline)
#   3. Ouvre une session IPPSSession avec -EnableSearchOnlySession
#   4. Recherche un nom disponible pour le Case (auto-incrément)
#   5. Crée le eDiscovery Case (conteneur de l'investigation)
#   6. Recherche un nom disponible pour le Hold (auto-incrément)
#   7. Crée la CaseHoldPolicy (qui) + CaseHoldRule (quoi)
#   8. Vérifie la création depuis la source de vérité
...
# On vérifie l'existence avant de continuer — cette vérification nécessite
# Connect-ExchangeOnline (pas IPPSSession, qui est un endpoint distinct).
...
# convention de nommage du lab.
# Note : Get-ComplianceCase supporte -Identity — lookup direct possible ici,
# contrairement à Get-UnifiedAuditLogRetentionPolicy (exo 6a) qui l'interdit.
...
# -CaseType "eDiscovery" : Standard. Autre valeur : "AdvancedEdiscovery" (Premium E5).
try {
...
# ========================================================================================
# ÉTAPE 5 : Création du Hold sur la mailbox (Policy + Rule)
# ========================================================================================
...
# --- CaseHoldPolicy ---
# -Case             : Case auquel ce hold est rattaché (obligatoire — pas de hold orphelin).
...
# On pourrait restreindre avec une query KQL : "CONFIDENTIEL" ne retiendrait que
# les éléments contenant ce mot-clé.
# Sans query = filet maximal — posture standard en investigation pour ne rien rater.
try {
...
# REX : 30 secondes minimum pour que les objets eDiscovery soient visibles
# après création — même latence de réplication que les DLP policies (exo 4b).
